...
export default [
  // 根据获取sku列表
  {
    ...
    response: (request) => {
      ...
      const allSkus = [...skuList].map((sku) => ({
        ...
        isSale: sku.isSale, // 默认所有SKU都在售
      }));

      // 计算分页
      const start = (page - 1) * limit;
      const end = start + limit;
      const records = allSkus.slice(start, end);
      ...
      //             skuDesc:"哈哈", // sku描述
      //             weight:"3.00", //sku重量
      //             tmId:3,  // 品牌(trademark)id
      //             category3Id:86, // 三级分类id
      //             skuDefaultImg:"http//",  // 图片地址
      //             isSale:1,  // 是否销售
      //             },
  ...
  // 获取商品详情
  {
    ...
    response: (request) => {
      ...
      // 转换SKU属性值列表
      const transformedAttrList = sku.skuAttrValueList.map(item => {
        // 查找属性名称
        const attr = attrList.find(a => a.id === item.attrId);
        // 查找属性值名称
        const attrValue = attrValueList.find(v => v.id === item.valueId);

        return {
          attrId: item.attrId,
          valueId: item.valueId,
          attrName: attr ? attr.attrName : "未知属性",
          valueName: attrValue ? attrValue.valueName : "未知值"
        };
      });

      // 转换销售属性值列表
      const transformedSaleAttrList = sku.skuSaleAttrValueList.map(item => {
        // 查找销售属性名称
        const saleAttr = allSaleAttrs.find(a => a.id == item.saleAttrId);
        let saleAttrValueName = "未知值";

        // 在销售属性数据中查找对应的值名称
        const spuSaleData = fixedSaleAttrData[sku.spuId] || fixedSaleAttrData.default;
        for (const attrGroup of spuSaleData) {
          const foundValue = attrGroup.spuSaleAttrValueList.find(
            v => v.id === item.saleAttrValueId
          );
          if (foundValue) {
            saleAttrValueName = foundValue.saleAttrValueName;
            break;
          }
        }
        ...
        return {
          ...
          saleAttrName: saleAttr ? saleAttr.name : "未知属性",
          saleAttrValueName: saleAttrValueName
        };
      });
      // 根据SPU ID获取图片列表
      const imageList = fixedImageData[sku.spuId] || fixedImageData.default;

      // 创建转换后的SKU对象
      const transformedSku = {
        ...sku,
        // 替换原来的skuDefaultImg为skuImageList
        skuImageList: imageList.map(img => ({
          id: img.id,
          imgName: img.imgName,
          imgUrl: img.imgUrl,
          // 设置默认图片（这里假设第一个图片是默认图片）
          isDefault: img.id === imageList[0].id ? 1 : 0
        })),
        skuAttrValueList: transformedAttrList,
        skuSaleAttrValueList: transformedSaleAttrList
      };

      // 删除原来的skuDefaultImg字段（如果需要保留，可以注释掉下面这行）
      delete transformedSku.skuDefaultImg;
  ...
  // 删除sku
  {
    ...
    response: (request) => {
      ...
      // 从skuList中删除该SKU
      const [deletedSku] = skuList.splice(skuIndex, 1);
